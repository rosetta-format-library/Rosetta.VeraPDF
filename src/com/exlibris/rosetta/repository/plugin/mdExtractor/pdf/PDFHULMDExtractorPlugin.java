...
public class PDFHULMDExtractorPlugin extends AbstractJhoveMDExtractorPlugin{
	...
	private String agentName ="veraPDF  ";
    ...
    private void validatePdf(String fileName) throws ModelParsingException, EncryptedPdfException, FileNotFoundException, ValidationException{
	    ...
	    if(result != null){
	    	...
	    	for (TestAssertion testAssertion : result.getTestAssertions()) {
				this.addError(testAssertion.getMessage());
			}
	    }
	    
	    //set Agent name
	    Iterator<ReleaseDetails> iterator =  ReleaseDetails.getDetails().iterator();
	    while(iterator.hasNext()){
	    	ReleaseDetails releaseDetails = iterator.next();
	    	agentName += ", " + StringUtils.capitalise(releaseDetails.getId()) + " Version "  + releaseDetails.getVersion();
	    }
    }
    

    public String getProfile(){
    	return result.getProfileDetails().getName();	    
    }
	...
	public String getAgent() {
		return "JHOVE , PDF-hul " + getRelease()+ " ; " +getAgentName() + " , Plugin Version " + pluginVersion;
	}
